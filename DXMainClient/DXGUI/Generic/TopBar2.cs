...
namespace DTAClient.DXGUI.Generic
{
    ...
    /// </summary>
    internal sealed class TopBar2 : XNAPanel
    {
        /// <summary>
        ...
        {
            if (!lanMode)
                SetSwitchButtonsClickable(promptWindow != PromptWindowEnum.Options);

            SetOptionsButtonClickable(promptWindow != PromptWindowEnum.Options);
        }
        ...
        private void ViewModelUpdated(CnCNetClientViewModel viewModel)
        {
            ...
            btnMainButton.Text = viewModel.PrimarySwitches[^1].GetSwitchName() + " (F2)";
        }

        private void SourcePanelUpdated(SourcePanel sourcePanel)
        {
            switch (sourcePanel)
            {
                case SourcePanel.Primary:
                    SwitchToPrimary();
                    return;
                case SourcePanel.Secondary:
                    SwitchToSecondary();
                    return;
                default:
