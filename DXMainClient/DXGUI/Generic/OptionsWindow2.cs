...
{
    public class OptionsWindow2 : XNAWindow
    {
        public OptionsWindow2(
        ...
        public override void Initialize()
        {
            ...
            {
                displayOptionsPanel, new AudioOptionsPanel(WindowManager, UserINISettings.Instance), new GameOptionsPanel(WindowManager, UserINISettings.Instance, topBar), new CnCNetOptionsPanel(WindowManager, UserINISettings.Instance, gameCollection), updaterOptionsPanel, componentsPanel
            };
            ...
            cncnetClientService.GetPromptWindow().Subscribe(PromptWindow);
        }

        private void ViewModelUpdated(CnCNetClientViewModel viewModel)
        {
            ToggleMainMenuOnlyOptions(viewModel.PrimarySwitches.Count == 1 && !viewModel.LanMode);

            if (viewModel.GameProcessState == GameProcessStateEnum.Starting)
                RefreshSettings();
        }

        private void PromptWindow(PromptWindowEnum promptWindow)
        {
            if (promptWindow != PromptWindowEnum.Options)
            {
                Disable();
                return;
            }
