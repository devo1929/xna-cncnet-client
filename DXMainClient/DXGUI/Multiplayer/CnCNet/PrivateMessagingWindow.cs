...
namespace DTAClient.DXGUI.Multiplayer.CnCNet
{
    public class PrivateMessagingWindow : XNAWindow, ISwitchable
    {
        ...
        {
            IEnumerable<IRCUser> userList = null;
            PlayerListBoxOptions options = new PlayerListBoxOptions();
            switch (tabControl.SelectedTab)
            {
                case MESSAGES_INDEX:

                    userList = privateMessageUsers.Select(u => u.IrcUser);
                    options = new PlayerListBoxOptions()
                    {
                        HighlightOnline = true
                    };
                    break;
                case FRIEND_LIST_VIEW_INDEX:
                    userList = cncnetUserData.FriendList.Select(friendName => connectionManager.UserList.Find(u => u.Name == friendName) ?? new IRCUser(friendName));
                    options = new PlayerListBoxOptions()
                    {
                        HighlightOnline = true,
                        HideFriendIcon = true
                ...
                case ALL_PLAYERS_VIEW_INDEX:
                    userList = connectionManager.UserList;
                    break;
            ...
            if (userList != null)
                lbUserList.UpdatePlayers(userList, options);
        }
